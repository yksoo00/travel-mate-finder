        ...
        kakao.maps.event.addListener(map, 'click', function(mouseEvent) {
            ...
            searchAddrFromCoords(latlng, function(result, status) {
                if (status === kakao.maps.services.Status.OK) {
                    ...
                    const addressDisplayField = document.getElementById('map-address-display');
                    const hiddenAddressField = document.getElementById('address');

                    if (addressDisplayField) addressDisplayField.value = addr;
                    if (hiddenAddressField) hiddenAddressField.value = addr;
